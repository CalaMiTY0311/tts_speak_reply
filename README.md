...
## 준비물
1. 캐릭터 TTS모델과 참조 오디오 (없다면 밑에 필자의 캐릭터 모델을 사용해주세요)
2. 좋은 CPU (안좋으면 느려요...)
    2-1 필자는 i5-11세대
...
2. [모델 폴더 다운URL](https://drive.google.com/file/d/1u9ockMEWKw1iTulXYzRGk8pq3DCS9MZb/view?usp=drive_link)(구글 드라이브 URL임...)에서 필자의 tts_models폴더를 다운받아서 최상위 디렉토리에 위치해주세요(잘 모르겠으면 api.py랑 같은 위치에 디렉토리를 위치 시켜주세요)
3. pip install -r requirements.txt 로 패키지 다운로드
4. python api.py로 서버 실행

### API 명세서
- [깃북](https://app.gitbook.com/o/S36xxi8gV9pHeLH2I2iI/s/mpnkOjEZrUEPuL0LUtLW/)에 소개와 실행방법을 문서화 해놓은 곳입니다. 참고하셔도 좋습니다.

## 참고 사항
- 텍스트(`text`)는 필수 입력값이며, 길어 질 수록 퀄리티가 낮아 질 수 있습니다. ( 추후 개선 예정 )
